...
# 避免 Windows / Git Bash 下 Python 默认 GBK 编码导致异常
...
SUMMARY_JSON="$(
PLAN="$PLAN" RESULTS_ROOT="$RESULTS_ROOT" python -c '
import json
import os
...
print(json.dumps({
    "total": len(jobs),
    "done": sum(1 for j in jobs if j["_done"]),
    "todo_indices": [j["_index"] for j in jobs if not j["_done"]],
    "phase": phase
}, ensure_ascii=False))
'
)"
...
TOTAL="$(
SUMMARY_JSON="$SUMMARY_JSON" python -c '
import json
import os
print(json.loads(os.environ["SUMMARY_JSON"])["total"])
'
)"
...
import json
import os
print(json.loads(os.environ["SUMMARY_JSON"])["done"])
'
)"
...
import json
import os
print(json.loads(os.environ["SUMMARY_JSON"])["phase"] or "")
'
)"
...
for idx in data["todo_indices"]:
    print(idx)
'
)"

TODO_INDICES=()
for idx in $TODO_TEXT; do
  TODO_INDICES+=("$idx")
done

echo "[INFO] Remaining jobs: ${#TODO_INDICES[@]}"
...
if [[ "${#TODO_INDICES[@]}" -eq 0 ]]; then
  ...
  for idx in "${TODO_INDICES[@]}"; do
    JOB_DONE="$(
PLAN="$PLAN" RESULTS_ROOT="$RESULTS_ROOT" JOB_INDEX="$idx" python -c '
import json
import os
